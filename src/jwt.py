...
def create_user():
	...
	if request.method == 'GET':
		...
		new_user: User = User(username=user['username'], password=user['password'])

		query: Query = db.session.query(User.id).filter_by(username=user['username'])
...
def delete_user():
	...
	query: Query = db.session.query(User).filter_by(username=username, password=password)
...
def login():
	if request.method == 'GET':
		...
		if found_user is not None and bcrypt.checkpw(password.encode('utf-8'), found_user.password):
...
def user():
	...
	return render_template('jwt/user.html', user=current_user,token=request.args.get('jwt'))
